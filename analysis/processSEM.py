...
for i in sorted(os.listdir(input_dir)):
    print(i)
    image=cv2.imread(input_dir+'/'+str(i))
    grayImage = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    rimage=removeStripesVertical(grayImage,level,wname,sigma)
    rimage_trim=rimage
    gaussian_blurred=ski.filters.gaussian(rimage_trim,sigma_blur)

    dilated2=gaussian_blurred-offset
    ...
    cv2.imwrite(output_dir+'/out_'+str(i), image2)
