...
class SEM_API:
    ...
    def __init__(self, state='local'):
        ...
        if res == 0:
            ...
            raise API_ERROR(res)
        self.Add_Event(self._wait_for_stage_idle)
        self.Set_Notify("DP_STAGE_IS")
    ...
    def MoveStage(self, coord):
        ...
        if len(coord) == 6:
            self._is_stage_moving = True
            res = self.mic.MoveStageDoubles(*coord)
            while self._is_stage_moving:
                time.sleep(0.2)
            #self.wait_for_stage_idle()
            return res
        else:
    ...
    def set_rotation(self, r):
        pos = self.GetStagePosition()
        self.MoveStage((pos[0], pos[1], pos[2], pos[3], r, pos[5]))
    ...
    def _wait_for_stage_idle(self, *args):
